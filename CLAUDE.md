...
| Loch *(virtuell)* | Ausgehobenes Grab in der Gartenmitte (`<g id="chain_7_grab">`) — perspektivisches Trapez-Loch (fu 0.34..0.66, fv 0.38..0.72) mit 4 Innen-Wänden (front hell-, back dunkel-braun; links heller, rechts dunkler — Lichtquelle oben-links) + dunklem Boden. Keine Umrandung, keine Erdklumpen. data-y-fuss=786. Sichtbar nach `chain_7_loch_offen=true`. Truhe sitzt HINTER dem Loch (nicht drin), Bottom-Edge an der hinteren Rim-Höhe y=684. | Inline-SVG-Gruppe |
...
**Cache-Busting** in `index.html`: aktuell `style.css?v=47`, `script.js?v=251`. Bei Änderungen an `script.js` oder `style.css` das `?v=N` hochzählen, sonst hängt die alte Version im Browser-Cache. Bei Änderungen an einem `<image href="assets/X.svg">`-Asset auch `?v=N` an den href anhängen — der Browser cached `<image>`-Sources separat. Gleiches gilt für SVGs, die per `drawImage` rasterisiert werden (z.B. `BUESCHE.hintenHalb`-`src` aktuell auf `assets/bush_3.svg?v=5`).
...
| 2 | Drag das andere Werkzeug auf gartenmitte_grab | Beide Flags true → `chain_7_loch_offen=true`, **`HINDERNISSE.garten.push(CHAIN_7_HINDERNIS)`** (Boden-Polygon fu 0.34..0.66, fv 0.38..0.72 — deckungsgleich mit dem visuellen Loch), `aktualisiereChain7()` zeigt `chain_7_grab` (Loch + Truhe dahinter). Story-Overlay „You break through the soil and uncover a wooden chest in the hole." |
...
**`chain_7_grab` (DOM):** `<g id="chain_7_grab" class="sanitar-aus" data-y-fuss="786">` enthält 5 Polygone (Front-Wand hell `#9c7548`, Back-Wand dunkel `#4a3522`, Linke Wand heller `#b08856`, Rechte Wand dunkler `#5a3f24`, Boden sehr dunkel `#2a1c10`) für ein 4-Wand-Loch mit Boden — Eckpunkte gemeinsam fürs perspektivische Trapez (fu 0.34..0.66, fv 0.38..0.72, ~440 px breit vorne, ~374 px hinten). KEINE Umrandung, KEINE Erdklumpen. Truhe `<image href="assets/chest_1.svg">` 170×76 sitzt HINTER dem Loch (x=715 y=608, Bottom-Edge bei y=684 = hintere Rim-Höhe). Initial via Klasse `sanitar-aus` versteckt; `aktualisiereChain7` togglet sie. Wird via `data-y-fuss` in den Front-Layer geklont (Tiefensortierung).
