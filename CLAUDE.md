...
| `chain_6_tulpe` (Hauptraum, plant_tulpe vorne-links, Polygon (20,840)..(175,895) — bewusst nur Topf+Stengel-Bereich, klar unter L-Tür-Bottom-Linie; laufziel fu=0.13 fv=0.04) | `chain_6_umfang` — Umfang | U = 2π·r | `schluesselteil_3` (Bart) |
...
- **Klick-Polygon vs. Tür-Polygon:** Die `pointerdown`-Pipeline ([script.js:4619](script.js:4619)) prüft Türen VOR Objekten — d.h. wenn ein OBJEKT-Polygon mit einem Tür-Polygon überlappt, gewinnt die Tür und der Spieler landet im Nachbarraum statt eine Aufgabe zu öffnen. Perspektivische Wand-Türen (`seitenTuerPolygon(linkeWandPunkt)` und `(rechteWandPunkt)`) reichen weit nach unten/innen, weil Wände schräg zum Fluchtpunkt verlaufen. Bekannt-betroffene Stellen (gefixt): `chain_6_tulpe` (Hauptraum) lag mit Center (97.5, 800) im L-Tür-Trapez → Polygon nach unten gezogen auf y=840..895 + Asset um 10 px nach unten verschoben (translate y=894); `bathtub` (Bad) hatte linke obere Ecke bei (130, 440) im buero-Tür-Trapez → linke Polygon-Kante auf x=220 verschoben. **Bei neuen Klick-Objekten in Wandnähe (linke/rechte Wand-Tür):** Polygon-Zentrum + alle Ecken gegen die Tür-Polygone testen, oder Asset weit genug von der Wand wegplatzieren. Der Drop-Pfad `versucheDrop` ([script.js:5435](script.js:5435)) ist davon nicht betroffen — er prüft Objekte mit passendem `akzeptiert[gegenstandId]` zuerst.
