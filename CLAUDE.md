...
**Cache-Busting** in `index.html`: aktuell `style.css?v=56`, `script.js?v=289`. Bei Änderungen an `script.js` oder `style.css` das `?v=N` hochzählen, sonst hängt die alte Version im Browser-Cache. Bei Änderungen an einem `<image href="assets/X.svg">`-Asset auch `?v=N` an den href anhängen — der Browser cached `<image>`-Sources separat. Gleiches gilt für SVGs, die per `drawImage` rasterisiert werden (z.B. `BUESCHE.hintenHalb`-`src` aktuell auf `assets/bush_3.svg?v=5`).
...
**Proximity-Fade Bürobild (Chain 5):** Die Loop-Musik kollidiert harmonisch mit den C/E/G-Tönen der Bürobild-Sequenz. `aktualisiereMusikProximity()` läuft jeden Frame in `loop()` und multipliziert den `musikGainNode.gain` abhängig von der Position der Figur zur Zone um `BUEROBILD_ANKER` (fu=0.05, fv=0.10, vorne-links neben der Lavalampe). Zone ist eine **Ellipse in (fu, fv)** mit Halbachsen `PROXIMITY_RX` (Breite, Default 0.30) und `PROXIMITY_RY` (Tiefe, Default 0.80) — `RY > RX`, damit die Ellipse in der Tiefen-Richtung gestreckt ist und auf dem Bildschirm nicht flach-breit wird. Optional `PROXIMITY_ROTATION` in Radian (Default 0.25 ≈ 14°) — kippt die Hauptachse Richtung +fu, sodass die Ellipse perspektivisch entlang der natürlichen Tiefenrichtung des Bürobild-Bereichs liegt. Innerhalb der inneren Ellipse (skaliert mit `PROXIMITY_NEAR_FRAC=0.25` der äusseren) komplett stumm, ausserhalb voll Musik, dazwischen linear. Glide via `setTargetAtTime(target, now, 0.15)` → ~0.45 s sanfter Ramp. Effekt nur aktiv, wenn `aktuellerRaum === "buero" && !bild_kreise_geloest`. Bei stillstehender Figur skipt die Funktion via `proximityLastMult`-Cache (Schwelle 0.01).
