...
| Loch *(virtuell)* | Ausgehobenes Grab in der Gartenmitte (`<g id="chain_7_grab">`) — perspektivisches Trapez-Loch (fu 0.34..0.66, fv 0.38..0.72) aus 3 vertikalen Wand-Trapezen, alle vom oberen Rim (y=684) bis zum unteren Rim (y=786). Mittelwand breiter (#9c7548 hell-braun), linke Wand schmaler heller (#b08856), rechte Wand schmaler dunkler (#5a3f24 — Lichtquelle oben-links). Keine Umrandung, kein Boden, keine Erdklumpen. data-y-fuss=786. Sichtbar nach `chain_7_loch_offen=true`. Truhe sitzt HINTER dem Loch (nicht drin), Bottom-Edge bei y=664 (= hintere Rim minus 20 px für leichten Abstand). | Inline-SVG-Gruppe |
...
**Cache-Busting** in `index.html`: aktuell `style.css?v=47`, `script.js?v=252`. Bei Änderungen an `script.js` oder `style.css` das `?v=N` hochzählen, sonst hängt die alte Version im Browser-Cache. Bei Änderungen an einem `<image href="assets/X.svg">`-Asset auch `?v=N` an den href anhängen — der Browser cached `<image>`-Sources separat. Gleiches gilt für SVGs, die per `drawImage` rasterisiert werden (z.B. `BUESCHE.hintenHalb`-`src` aktuell auf `assets/bush_3.svg?v=5`).
...
**`chain_7_grab` (DOM):** `<g id="chain_7_grab" class="sanitar-aus" data-y-fuss="786">` enthält 3 Polygone — drei vertikale Wand-Trapeze, die das Loch füllen (linke `#b08856` heller, mittlere `#9c7548` hell-braun + breiter, rechte `#5a3f24` dunkler). Alle Wände gehen vom oberen (y=684) bis zum unteren Rim (y=786). KEIN Boden, KEIN Back-Wall-Polygon, KEINE Umrandung, KEINE Erdklumpen. Truhe `<image href="assets/chest_1.svg">` 170×76 sitzt HINTER dem Loch (x=715 y=588, Bottom bei y=664). Initial via Klasse `sanitar-aus` versteckt; `aktualisiereChain7` togglet sie. Wird via `data-y-fuss` in den Front-Layer geklont (Tiefensortierung).
