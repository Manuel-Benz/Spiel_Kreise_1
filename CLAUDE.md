...
- **Schreibtischstuhl** `#chair_1` (Inline-SVG): vor Tisch 2 (im DOM nach Tisch = visuell davor), `x=450, y=400, 180×290`, `overflow="visible"`. Spiegelung an y-Mittelachse über ein **inneres** `<g transform="matrix(-1 0 0 1 52.90741 0)">` (= Mirror um die viewBox-x-Mitte 52.90741/2), das `<g id="layer1">` umschliesst. Eigene Saturierung: `filter="url(#grell-mild)"`. Manuelle Säuberung der SVG: `path1545` (Detail) entfernt; alle übrigen Pfade haben `stroke = ihr Fill` mit `stroke-width:1` (puffen sich minimal auf, keine schwarze Outline). Kein eigenes Hindernis (im Footprint des Tisches). **Früher:** outer-`<svg>`-Transform `matrix(-1 0 0 1 580 0)` mit `x=-50` — SVG-2-Verhalten, das beim Hide/Show des Raums in manchen Browsern intermittent verrutschte (Stuhl tauchte plötzlich in der linken Wand auf). Inner-`<g>`-Mirror ist SVG-1.1-konform und stabil.
...
**Nachtsicht-Toggle via Inventar-Klick:** Pickup zeigt Story-Text „You take the binoculars and the world shifts into phosphor-green night vision. Tap the binoculars in your inventory to toggle the effect." Ein **Klick** (kein Drag) auf den Binoculars-Inventar-Slot ruft `toggleNachtsicht()` → schaltet die Body-Klasse + den `.aktiv`-Glow am Slot ein/aus. Click-vs-Drag-Erkennung über `KLICK_DRAG_SCHWELLE_PX = 6` in `starteDrag` (Pointer-Bewegung < 6 px = Klick → `klickeInventarItem(id)`, sonst Drag → `versucheDrop`). Der `.inventar-slot.aktiv`-Glow ist phosphor-grün (`#5fff8a` Border + Box-Shadow, gleicher Farbton wie die Geheimtür-Outline) und wird direkt in `aktiviereNachtsicht`/`deaktiviereNachtsicht` mitgesetzt — so synchronisiert er sich auch beim Reload, wenn `aktualisiereInventar()` vor der Nachtsicht-Reaktivierung in `aktualisiereAllesNachLaden` läuft.
...
**Reset-Zahnrad + Settings-Menü** (unten rechts auf der Stage, links neben dem Fullscreen-Button):
- Klick aufs Zahnrad togglet das Settings-Menü (Sound on/off, Music on/off, Reset progress).
- Sound-/Musik-Toggle: schreibt `soundAn` / `musikAn` um, ruft `speicherEinstellungen()` (separater Save unter `SETTINGS_KEY`), aktualisiert die Status-Anzeige (grün=an, rot=aus). `starteMusik()` und `spieleAudio()` prüfen die Flags intern → Toggle wirkt sofort, auch wenn er VOR dem ersten in-Game-Klick auf dem Begrüssungsbildschirm gemacht wird.
...
- Slot-Layout (von rechts nach links): Fullscreen (`right: 1.5%`) → Zahnrad (`right: calc(1.5% + 44px + 8px)`). Settings-Menü ist oberhalb des Zahnrads verankert und zieht mit, wenn die Slot-Breite je geändert wird.
- **z-index-Hierarchie** (Zahnrad/Fullscreen 21, Settings-Menü 22, Reset-Overlay 25, Start-/Sieg-Overlay 20): Zahnrad + Fullscreen sind auf ALLEN Bildschirmen erreichbar (Begrüssungsbildschirm, In-Game, Sieg-Endscreen). Music/Sound-Toggle ist also schon vor dem ersten Klick zugänglich. Reset-Overlay liegt darüber, damit der Bestätigungs-Dialog auch beim Auslösen vom Begrüssungs-/Sieg-Screen aus alles deckt.

**Fullscreen-Button** `#fullscreen-button` (ganz rechts unten auf der Stage):
- Togglet `document.documentElement.requestFullscreen()` / `exitFullscreen()`. Icon (4 Ecken nach aussen) bleibt im Kreis stehen — kein Icon-Swap; nur `aria-label` und `title` wechseln per `fullscreenchange`-Listener zwischen „Fullscreen" und „Exit fullscreen".
- Feature-Detect über `document.fullscreenEnabled || document.webkitFullscreenEnabled` — bei nicht unterstützenden Browsern (iPhone Safari) wird der Button per `hidden=true` ausgeblendet (graceful degradation, Stage rückt nicht nach).
- Webkit-Fallback (`webkitRequestFullscreen` / `webkitExitFullscreen` / `webkitfullscreenchange`) ist mit drin, falls je ältere Safari-Versionen ins Spiel kommen — moderne iPadOS/Safari nutzen die Standard-API.
- **z-index 21** (über Start-Overlay und Sieg-Overlay, beide z-index 20) → Fullscreen ist schon vom Begrüssungsbildschirm erreichbar und bleibt im Sieg-Endscreen klickbar. Zahnrad ebenfalls z-index 21 (siehe „Reset-Zahnrad + Settings-Menü" oben).
- Bestehende `resizeCanvas()`-Logik (grösstes 16:9-Rechteck im Viewport) handhabt den Wechsel automatisch — kein Spezialcode nötig.
...
| 1→2a | Silber-Schlüssel auf cupboard_1 (linke Hälfte, x=980..1180) gezogen | **MC-Aufgabe `chain_1_schloss` öffnet sich** (90° → π/2 rad). Bei richtig: `verbrauche("schluessel_buero")` + `oeffneCupboard1()` → Schrank-Switch öffnet. Bei falsch oder Abbruch (×/Esc): Schlüssel BLEIBT im Inventar, Spieler kann erneut droppen → Aufgabe öffnet sich wieder (8 s-Lockout greift wie üblich, ist aber recoverable). |
...
**Auto-Close DEAKTIVIERT (User-Entscheidung):** Overlays schliessen sich NICHT mehr automatisch — der User schliesst per ×, Esc oder Backdrop-Klick. Die Funktion `automatischSchliessen()` ist ein No-op (clear pending timer for safety, sonst nichts), damit alle ~17 bestehenden Call-Sites unverändert weiterlaufen. Wer Auto-Close je wieder aktivieren möchte, ändert nur die eine Funktion zurück auf `setTimeout(schliesseOverlay, ms)` — alle Call-Sites greifen automatisch wieder.
...
| `chain_6_tulpe` (Hauptraum, plant_tulpe vorne-links, Polygon (15,730)..(180,895) — deckt die GANZE sichtbare Tulpe; `vorTueren: true` lässt den Klick die L-Tür im Überlapp schlagen; laufziel fu=0.13 fv=0.04) | `chain_6_umfang` — Umfang | U = 2π·r | `schluesselteil_3` (Bart) |
...
1. Beim 4ten erfolgreich gelösten Pickup ruft die Aufgabe `sammleSchluesselteil(id)` → `linkesInventar.add(id)` → `aktualisiereLinkesInventar()`. Bei Set-Size 4 → `setTimeout(kombiniereSchluessel, 4000)` — Animation startet 4 s nach dem Lösen der 4. Aufgabe. Da Auto-Close global deaktiviert ist, läuft die Animation im Hintergrund hinter dem noch offenen Belohnungs-Overlay; Spieler sieht sie, sobald er das Overlay manuell schliesst.
...
3. Nach 1.5 s: `linkesInventar.clear()`, `gegenstaende.add("vereinter_schluessel")`, beide Inventare aktualisieren, Story-Overlay "The three key fragments and the glue fuse into one complete key" (manuell schliessen — Auto-Close global deaktiviert).
...
- Klick auf Slot → `pointerdown` mit `e.preventDefault()` → `zeigeOverlayText("Collection items on the left can't be used for interactions — only inventory items on the right can.")` (manuell schliessen — Auto-Close global deaktiviert). Kein Drag-Start.
...
- **Loop** (`<Raum>_2`-Files, je **9 Takte**: 8 Takte Musik + 1 Takt Hall/Auslauf am Schluss): ~300 ms vor dem virtuellen Track-Ende (`MUSIK_LOOKAHEAD`) liest `decideUndPlane()` den aktuellen Raum (`aktuellerRaum`) und plant das passende `_2` (`Haupt_2`/`Buero_2`/`Badezimmer_2`/`Garten_2`/`Keller_2`) exakt am virtuellen Endezeitpunkt des laufenden Tracks (`source.start(endTime)`). **Virtuelles Ende = Buffer-Ende − 1 Takt** (siehe Crossover-Logik unten) → der 1. Takt des neuen Files läuft unter dem 9. Takt Hall des alten → nahtloser Übergang, Hall verschwimmt natürlich in die neue Musik. Raumwechsel mid-Loop wirkt sich erst beim Decision-Point aus — die laufenden 8 Takte spielen aus, der Hall geht als 1. Takt des neuen Raums weiter.
- **Outro** (`Garten_3`): wenn `chain_7_geoeffnet === true` (Truhe geöffnet → Sieg-Overlay) und Decision-Point für nächsten Track läuft, wird `Garten_3` statt `Garten_2` geplant (Übergang ebenfalls per Hall-Crossover, da der laufende Track ein _2 ist). Danach Phase `done`, kein weiteres Audio.
...
**Crossover-Logik** (`trackEndOffset(name)`): Pro Track wird ein End-Offset berechnet, um den der nächste Track früher startet als das Buffer-Ende. Zwei Pfade:
- **Loop-Files (`*_2`)**: Offset = `TAKT_SEKUNDEN = 60 / 120 * 4 = 2.0 s` (1 Takt bei 120 BPM, 4/4). Damit überlappt der 9. Takt Hall des aktuellen _2 mit dem 1. Takt des nächsten Tracks. Funktioniert sowohl für _2 → _2 (Loop-Wechsel) als auch _2 → `Garten_3` (Outro).
- **Intro/Outro (`Haupt_1`, `Garten_3`)**: Offset = `MP3_PADDING_KOMPENSATION = 0.08 s` (kein Hall-Tail; nur das MP3-Encoder-Padding kompensieren, damit die ~26 ms Trailing-Stille des aktuellen mit der Leading-Stille des nächsten überlappt).

**MP3-Padding-Workaround** (`MP3_PADDING_KOMPENSATION = 0.08` s): MP3-Encoder fügen am Anfang/Ende jedes Files Padding-Samples ein (~1100 Leading + ~1152 Trailing = ~26 ms je). Selbst mit sample-genauem Scheduling hört man dadurch eine kleine Pause zwischen Tracks. Wird nur für Tracks ohne eigenen Hall-Tail verwendet (Intro/Outro) — bei _2-Files ist der 1-Takt-Crossover ohnehin viel grösser als das Padding und schluckt es mit. Falls Files mit anderem Encoder kodiert werden, Wert empirisch tunen.
...
**Debug-Visualisierung der Proximity-Zone:** `musikProximityDebug(true)` in der Konsole zeigt zwei rote Ellipsen auf dem Boden (nur im Büro): Innere (kräftiges Rot) = NEAR-Grenze (vollständig stumm), Äussere (transparentes Rot) = FAR-Grenze (Übergang zu voll laut). Anker als weisser Punkt mit rotem Rand. Werte zur Laufzeit tunbar via `setMusikProximity({ fu, fv, rx, ry, nearFrac, rotation })` (alle Felder optional) — Debug-Zone updatet im nächsten Frame.
...
- **Klick-Polygon vs. Tür-Polygon:** Die `pointerdown`-Pipeline prüft Türen VOR Objekten — d.h. wenn ein OBJEKT-Polygon mit einem Tür-Polygon überlappt, gewinnt die Tür und der Spieler landet im Nachbarraum statt eine Aufgabe zu öffnen. Perspektivische Wand-Türen (`seitenTuerPolygon(linkeWandPunkt)` und `(rechteWandPunkt)`) reichen weit nach unten/innen, weil Wände schräg zum Fluchtpunkt verlaufen. **Lösungs-Pattern**: Objekte mit dem Flag `vorTueren: true` greifen auf zwei Ebenen: (a) der Helper `findePrioObjektBei` checkt sie im pointerdown vor `findeTuerBei` und löst die Aktion aus, sobald das Objekt aktiv ist; (b) `findeTuerBei` filtert Türen, deren Treffer in einem `vorTueren`-Polygon liegt, IMMER raus — UNABHÄNGIG vom `aktiv`-State des Objekts. Damit bleibt die Tür im Überlapp auch dann blockiert, wenn das Objekt selbst noch nicht freigeschaltet ist (z.B. Tulpe vor Formelbuch — sonst würde die L-Tür im Tulpen-Bereich klickbar wirken, bevor das Spiel die Tulpe als Pickup-Stelle bewirbt). Cursor-Feedback in pointermove zieht symmetrisch nach. Der Drop-Pfad `versucheDrop` prüft sowieso Objekte vor Türen, davon also nicht betroffen. Bekannt-genutzte Stellen: `chain_6_tulpe` (Hauptraum) — Klick-Polygon (15,730)..(180,895) deckt die ganze Tulpe inkl. Blüte, überlappt absichtlich mit dem L-Tür-Trapez im unteren Bereich. Frühere Stellen, die ohne `vorTueren` gefixt wurden (Polygon kleiner ziehen statt Prio): `bathtub` (Bad) hatte linke obere Ecke bei (130, 440) im buero-Tür-Trapez → linke Polygon-Kante auf x=220 verschoben. Faustregel: Bei neuen Klick-Objekten in Wandnähe (linke/rechte Wand-Tür) entweder `vorTueren: true` setzen oder Polygon-Zentrum + alle Ecken gegen die Tür-Polygone testen.
...
- **`<svg>`-Klick-Polygone für Chain 7 mussten gross dimensioniert werden:** Die Drop-Targets `gartenmitte_grab` (560..1040, 660..800 = 480×140) und `chest_1` (700..900, 590..680 = 200×90 direkt auf der Truhe hinter dem Loch) sind absichtlich grosszügig, damit der User das mit dem Drag-Cursor präzise treffen muss.
...
- **„Start over"-Skip-Flag (sessionStorage):** Damit „Start over" → Confirm direkt ins Spiel führt (statt wieder den Begrüssungsbildschirm zu zeigen), wird beim Confirm zusätzlich `sessionStorage["spiel_kreise_1_skip_start"]="1"` gesetzt, dann reloaded. **Inline-`<script>` im `<head>`** prüft das Flag synchron beim nächsten Load (vor Body-Parse), konsumiert es, setzt `html.skip-start`-Klasse → CSS versteckt den Overlay sofort, kein 1-Frame-Aufblitzen. `zeigeStartScreen()` prüft die Klasse statt sessionStorage. SessionStorage statt localStorage, damit das Flag nicht über Tabs/Sessions hinweg „klebt".
- **No-Flicker bei normalem Reload:** `#start-overlay` ohne `hidden`-Attribut → sichtbar von t=0 an. Vorher: `hidden` → Browser malte zuerst Inline-SVG-Möbel (Canvas noch leer) → JS lief, zeichnete Canvas, dann erst Overlay → sichtbarer FOUC. Jetzt: Overlay deckt von Anfang an alles ab, der Spiel-Aufbau dahinter ist unsichtbar.
- **`transform`-Attribut auf nested `<svg>` ist SVG 2** und kann beim Hide/Show des `<g data-raum>` (Raumwechsel) intermittent verrutschen — Element rendert dann an seiner unverdrehten Bbox-Position. Beobachtet bei chair_1 (Bürostuhl tauchte ohne Mirror in der linken Wand auf). **Lösung:** Mirror-/Rotations-Transforms NICHT auf das outer `<svg>` legen, sondern auf ein **inneres `<g>`**, das den Inhalt umschliesst. Outer-`<svg>` x/y/width/height passend wählen, sodass das Element schon ohne Transform am richtigen Bildschirm-Ort sitzt. `<g>`-Transforms sind SVG-1.1-konform und in allen Browsern stabil. Falls die gespiegelten Pfade über die viewBox hinausragen, zusätzlich `overflow="visible"` am outer `<svg>`.
- **Item nicht beim Drop verbrauchen, wenn der Drop eine Aufgabe öffnet** (Schlüssel-Verlust-Bug bei cupboard_1): Wird das Item im `akzeptiert`-Callback per `verbrauche()` entfernt UND dann nur eine Aufgabe geöffnet (`zeigeAufgabe(...)`), verliert der Spieler das Item bei falscher Antwort + Auto-Close (8 s-Lockout schliesst das Overlay nach 2 s) endgültig — der Drop-Target ist dann inaktiv (Item nicht mehr im Inventar) und es gibt keinen Weg, die Aufgabe nochmal zu öffnen. **Regel:** Wenn der Drop nur eine Aufgabe triggert, das Item ERST in `bei_richtig.callback` der Aufgabe via `verbrauche()` entfernen. Bei falscher Antwort oder Schliessen ohne Antwort bleibt das Item im Inventar → Spieler kann erneut droppen + retry'en. Etablierter Pattern in chain_2_octopus / chain_3_pizza / chain_1_pi / chain_4_teppich. cupboard_1 war historisch die einzige Ausnahme (jetzt gefixt).
...
**Item-Verbrauch:** Die Helpers räumen Items auf, die im echten Spielfluss beim entsprechenden Drop verbraucht würden — `bridge()` löscht `code_geheimtuer` + `binoculars_1` + den `keller_code`-Inventar-Eintrag (alle drei wären nach erfolgreichem Geheimtür-Drop weg); `chain7()` löscht `schaufel` + `pickel` (verbraucht beim grab-Drop). Damit ergibt z.B. `chain1()` + `bridge()` + `chain4()` + `chain5()` + `chain7()` denselben Inventar-Endstand wie das vollständige UI-Spiel. `chain6()` erzeugt direkt den `vereinter_schluessel` (Combine-Endzustand) und leert `linkesInventar`.
