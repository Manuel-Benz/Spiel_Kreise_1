...
| Loch *(virtuell)* | Ausgehobenes Grab in der Gartenmitte (`<g id="chain_7_grab">`) — perspektivisches Trapez-Loch (fu 0.34..0.66, fv 0.38..0.72): Back-Wand (`#4a3522` dunkel braun) als Strip am oberen Rim + 3 vertikale Wand-Trapeze darunter (links `#b08856` heller, mitte `#9c7548` hell-braun + breiter, rechts `#5a3f24` dunkler — Lichtquelle oben-links), alle drei vom inner-back y=720 bis zum unteren Rim y=786. Keine Umrandung, kein Boden, keine Erdklumpen. data-y-fuss=786. Sichtbar nach `chain_7_loch_offen=true`. Truhe sitzt HINTER dem Loch (nicht drin), x=715 y=598. | Inline-SVG-Gruppe |
...
**Cache-Busting** in `index.html`: aktuell `style.css?v=47`, `script.js?v=253`. Bei Änderungen an `script.js` oder `style.css` das `?v=N` hochzählen, sonst hängt die alte Version im Browser-Cache. Bei Änderungen an einem `<image href="assets/X.svg">`-Asset auch `?v=N` an den href anhängen — der Browser cached `<image>`-Sources separat. Gleiches gilt für SVGs, die per `drawImage` rasterisiert werden (z.B. `BUESCHE.hintenHalb`-`src` aktuell auf `assets/bush_3.svg?v=5`).
...
**`chain_7_grab` (DOM):** `<g id="chain_7_grab" class="sanitar-aus" data-y-fuss="786">` enthält 4 Polygone: Back-Wand `#4a3522` (Strip am oberen Rim, y=684 bis y=720) und drei vertikale Wand-Trapeze darunter (linke `#b08856` heller, mittlere `#9c7548` hell-braun + breiter, rechte `#5a3f24` dunkler), die alle vom inner-back y=720 bis zum unteren Rim y=786 reichen. KEIN Boden, KEINE Umrandung, KEINE Erdklumpen. Truhe `<image href="assets/chest_1.svg">` 170×76 sitzt HINTER dem Loch (x=715 y=598). Initial via Klasse `sanitar-aus` versteckt; `aktualisiereChain7` togglet sie. Wird via `data-y-fuss` in den Front-Layer geklont (Tiefensortierung).
